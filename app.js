 ...
 * Main Application Logic v3.9.4 - Multi-coin view, TF price info, OHLCV colors
 */
...
function renderAddedCoins() {
    const container = document.getElementById('addedCoinsColumns');
    if (!container) return;
    ...
    state.addedCoins.forEach(symbol => {
        ...
        // Create full coin column
        const column = document.createElement('div');
        column.className = 'added-coin-column';
        column.dataset.symbol = symbol;
        ...
        column.innerHTML = `
            ...
            <div class="added-coin-charts" id="addedCharts-${symbol}">
                <!-- Timeframe rows will be added here -->
            </div>
        `;
...
function renderAddedCoinTimeframeRow(symbol, coin, tf, candles) {
    ...
    row.innerHTML = `
        <div class="tf-info">
            ...
            <div class="tf-price">$${latestCandle.close.toFixed(coin.decimals)}</div>
        </div>
        <div class="tf-chart-wrapper">
            <canvas class="tf-chart" id="${canvasId}"></canvas>
        </div>
    `;
