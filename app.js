 ...
 * Main Application Logic v3.9.5 - Multi-coin view, TF price info, OHLCV colors
 */
...
function renderAddedCoins() {
    const container = document.getElementById('addedCoinsPanels');
    if (!container) return;
    ...
    state.addedCoins.forEach(symbol => {
        ...
        // Create full coin panel (same structure as main)
        const panel = document.createElement('div');
        panel.className = 'added-coin-panel';
        panel.dataset.symbol = symbol;
        
        // Panel with header and charts
        panel.innerHTML = `
            <div class="added-coin-header">
                <div class="added-coin-info">
                    <img class="added-coin-logo" src="${logoUrl}" onerror="this.onerror=null;this.src='${getDefaultCoinLogo()}';">
                    <span class="added-coin-name">${coin.name}</span>
                </div>
                <div class="added-coin-price" id="addedPrice-${symbol}">
                    <span class="price-value">Loading...</span>
                </div>
                <button class="added-coin-remove" onclick="removeAddedCoin('${symbol}')" title="Remove">✕</button>
            </div>
            <div class="added-coin-charts" id="addedCharts-${symbol}">
                <div class="loading-indicator">Loading timeframes...</div>
            </div>
        `;
...
async function fetchAddedCoinFullData(symbol, coin) {
    try {
        ...
        // Fetch candles for each enabled timeframe
        const enabledTFs = state.timeframes.filter(tf => tf.enabled);
        
        for (const tf of enabledTFs) {
            await fetchAddedCoinTimeframe(symbol, coin, tf);
        }
...
function renderAddedCoinTimeframeRow(symbol, coin, tf, candles) {
    const chartsContainer = document.getElementById(`addedCharts-${symbol}`);
    ...
    row.className = `tf-row added-tf-row ${dirClass}`;
    row.innerHTML = `
        <div class="tf-info">
            <div class="tf-name-row">
                <span class="tf-name">${tf.label}</span>
            ...
            <div class="tf-timer" id="tf-timer-${symbol}-${tf.id}">${formatTime(secondsToClose)}</div>
            <div class="tf-price-mini">$${latestCandle.close.toFixed(coin.decimals)}</div>
            <div class="tf-change-mini ${changePct >= 0 ? 'positive' : 'negative'}">${changePct >= 0 ? '+' : ''}${changePct.toFixed(2)}%</div>
        </div>
        <div class="tf-chart-wrapper">
            <div class="tf-chart-container">
